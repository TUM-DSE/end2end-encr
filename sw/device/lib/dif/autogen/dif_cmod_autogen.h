...
#ifndef OPENTITAN_SW_DEVICE_LIB_DIF_AUTOGEN_DIF_CMOD_AUTOGEN_H_
#define OPENTITAN_SW_DEVICE_LIB_DIF_AUTOGEN_DIF_CMOD_AUTOGEN_H_
 ...
 */
typedef struct dif_cmod {
  /**
   * The base address for the cmod hardware registers.
   */
  mmio_region_t base_addr;
} dif_cmod_t;

/**
 * Creates a new handle for a(n) cmod peripheral.
 *
 * This function does not actuate the hardware.
 *
 * @param base_addr The MMIO base address of the cmod peripheral.
 * @param[out] cmod Out param for the initialized handle.
 ...
 * A cmod alert type.
 */
typedef enum dif_cmod_alert {
  /**
   * This fatal alert is triggered when a fatal TL-UL bus integrity fault is
   * detected.
   */
  kDifCmodAlertFatalFault = 0,
} dif_cmod_alert_t;
 ...
 *
 * This is an opaque type, to be used with the `dif_cmod_irq_get_state()`
 * function.
 */
typedef uint32_t dif_cmod_irq_state_snapshot_t;
 ...
 * `dif_cmod_irq_disable_all()` and `dif_cmod_irq_restore_all()`
 * functions.
 */
typedef uint32_t dif_cmod_irq_enable_snapshot_t;
 ...
 */
OT_WARN_UNUSED_RESULT
dif_result_t dif_cmod_irq_disable_all(const dif_cmod_t *cmod,
                                      dif_cmod_irq_enable_snapshot_t *snapshot);
...
#endif  // OPENTITAN_SW_DEVICE_LIB_DIF_AUTOGEN_DIF_CMOD_AUTOGEN_H_
