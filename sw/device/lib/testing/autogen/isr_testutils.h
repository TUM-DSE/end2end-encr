...
/**
 * A handle to a cmod ISR context struct.
 */
typedef struct cmod_isr_ctx {
  /**
   * A handle to a cmod.
   */
  dif_cmod_t *cmod;
  /**
   * The PLIC IRQ ID where this cmod instance's IRQs start.
   */
  dif_rv_plic_irq_id_t plic_cmod_start_irq_id;
  /**
   * The cmod IRQ that is expected to be encountered in the ISR.
   */
  dif_cmod_irq_t expected_irq;
  /**
   * Whether or not a single IRQ is expected to be encountered in the ISR.
   */
  bool is_only_irq;
...
/**
 * Services an cmod IRQ.
 *
 * @param plic_ctx A PLIC ISR context handle.
 * @param cmod_ctx A(n) cmod ISR context handle.
 * @param[out] peripheral_serviced Out param for the peripheral that was
 * serviced.
 * @param[out] irq_serviced Out param for the IRQ that was serviced.
 */
